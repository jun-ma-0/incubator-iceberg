...
class AssignFreshIds extends TypeUtil.CustomOrderSchemaVisitor<Type> {
  ...
  public Type struct(Types.StructType struct, Iterable<Type> futures) {
    ...
    for (int i = 0; i < length; i += 1) {
      ...
      if (field.isOptional()) {
        newFields.add(Types.NestedField.optional(newIds.get(i), field.name(), type, field.doc(), field.bfConfig()));
      } else {
        newFields.add(Types.NestedField.required(newIds.get(i), field.name(), type, field.doc(), field.bfConfig()));
      }
    }
