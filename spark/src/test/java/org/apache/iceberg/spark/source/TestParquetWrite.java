...
public class TestParquetWrite {
  ...
  public void testBasicWrite() throws IOException {
    ...
    for (ManifestFile manifest : table.currentSnapshot().manifests()) {
      for (DataFile file : ManifestReader.read(localInput(manifest.path()), null, table.io())) {
  ...
  public void testBasicWriteMap() throws IOException {
    ...
    List<SimpleRecordWithMap> actual = result.orderBy("id").as(Encoders.bean(SimpleRecordWithMap.class))
        .collectAsList();
    Assert.assertEquals("Number of rows should match", expected.size(), actual.size());
    Assert.assertEquals("Result rows should match", expected, actual);
    for (ManifestFile manifest : table.currentSnapshot().manifests()) {
      for (DataFile file : ManifestReader.read(localInput(manifest.path()), table.io())) {
        Assert.assertNotNull("Split offsets not present", file.splitOffsets());
        Assert.assertEquals("Should have reported record count as 1", 1, file.recordCount());
        Assert.assertNotNull("Column sizes metric not present", file.columnSizes());
        Assert.assertNotNull("Counts metric not present", file.valueCounts());
        ...
        String bfBasePath = LocationProvider.getBloomFilterBaseLocationFromTablePath(
            table.location(), file.path().toString(), spec, file.partition());
        String bfPath = String.format("%s-1", bfBasePath);
        InputFile bfFile = localInput(bfPath);
        Assert.assertTrue("Bloom Filter file should exist", bfFile.exists());
      }
    }
  }
  ...
  @Test
  public void testAppendWithSnapshotTag() throws IOException {
    File parent = temp.newFolder("parquet");
    File location = new File(parent, "test");
    ...
    // key & value have to be in lower cases
    String tag = "tags.key";
    String tagWithoutPrefix = "key";
    String tagValue1 = "test.value.1";
    String tagValue2 = "test.value.2";
    df.select("id", "data").write()
        .format("iceberg")
        .mode("append")
        .option(tag, tagValue1)
        .save(location.toString());

    df.withColumn("id", df.col("id").plus(3)).select("id", "data").write()
        .format("iceberg")
        .mode("append")
        .option(tag, tagValue2)
        .save(location.toString());
    ...
    Assert.assertTrue(currentSnapshot.summary().containsKey(tagWithoutPrefix));
    Assert.assertTrue(currentSnapshot.summary().get(tagWithoutPrefix).equals(tagValue2));

    Snapshot parentSnapshot = table.snapshot(table.currentSnapshot().parentId());
    Assert.assertTrue(parentSnapshot.summary().containsKey(tagWithoutPrefix));
    Assert.assertTrue(parentSnapshot.summary().get(tagWithoutPrefix).equals(tagValue1));
  }
