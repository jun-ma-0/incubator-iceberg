...
public class HadoopTombstoneExtension implements TombstoneExtension {
  ...
  public OutputFile remove(Snapshot snapshot, List<Entry> entries, Namespace namespace) {
    ...
    try {
      ...
      throw new RuntimeIOException(String.format("Failed to write tombstones to file: %s", outputFile.location()), e);
    }
  }

  @Override
  public OutputFile append(Snapshot snapshot, List<Entry> entries, Namespace namespace, Map<String, String> props) {
    OutputFile outputFile = newTombstonesFile(ops.current());
    List<Tombstone> current = load(snapshot);
    try {
      // All entries share the same addedOn property as the number of millis since the epoch of 1970-01-01T00:00:00Z
      current.addAll(fromExternal(entries, Instant.now().toEpochMilli(), props, namespace));
      new Tombstones(this.conf).write(current, outputFile.location());
    } catch (IOException | URISyntaxException e) {
  ...
  private List<Tombstone> load(Snapshot snapshot) {
    ...
    if (file.isEmpty()) {
      LOG.debug("No available tombstones, invalid snapshot summary property: {}", SNAPSHOT_TOMBSTONE_FILE_PROPERTY);
      return Lists.newArrayList();
    }
    try {
      return new Tombstones(conf).load(file);
    } catch (IOException e) {
      throw new RuntimeIOException(String.format("Failed to read tombstones from file: %s", file), e);
    }
  }
