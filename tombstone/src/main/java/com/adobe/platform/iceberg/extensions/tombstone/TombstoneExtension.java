...
public interface TombstoneExtension {
  ...
  /**
   * Appends current tombstones entries with the provided tombstones and writes result into new file.
   *
   * @param snapshot the table snapshot
   ...
   */
  OutputFile append(Snapshot snapshot, List<Entry> entries, Namespace namespace, Map<String, String> properties);
  ...
  /**
   * Copies the referenced tombstone file (where available) from the current snapshot to the new snapshot along with
   * appending the referenced files as an atomic commit operation.
   *
   * @param snapshot the table snapshot
